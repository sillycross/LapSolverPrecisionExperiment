...
{
	function<SolverReturnValue(const Vec&,FLOAT,int,const Vec&)> fn;
	AbstractSolver(const function<SolverReturnValue(const Vec&,FLOAT,int,const Vec&)> &fn): fn(fn) {};
	SolverReturnValue solve(const Vec &b, FLOAT tol=1e-6, int maxit=-1, const Vec &x0=Vec(0)) const 
	{ 
		if (x0.n==0) 
			return fn(b,tol,maxit,Vec(b.n)); 
		else 
		{ 
			assert(x0.n==b.n); 
			return fn(b,tol,maxit,x0); 
		}
	}
};
